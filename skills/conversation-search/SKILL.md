...
python ~/.claude/skills/conversation-search/scripts/search_history.py --digest today

# Yesterday's digest
python ~/.claude/skills/conversation-search/scripts/search_history.py --digest yesterday

# Specific date
python ~/.claude/skills/conversation-search/scripts/search_history.py --digest 2026-01-04

# Filter to specific project
python ~/.claude/skills/conversation-search/scripts/search_history.py --digest today --project "C:\Users\ee01287\Documents\Projects\Krok"
```
...
python ~/.claude/skills/conversation-search/scripts/search_history.py --today "newsletter"

# Search yesterday
python ~/.claude/skills/conversation-search/scripts/search_history.py --yesterday "bug fix"

# Search last N days
python ~/.claude/skills/conversation-search/scripts/search_history.py --days 7 "refactor"

# Search since a specific date
python ~/.claude/skills/conversation-search/scripts/search_history.py --since 2026-01-01 "feature"
```

## Full Usage

```bash
python ~/.claude/skills/conversation-search/scripts/search_history.py "<query>" [options]
python ~/.claude/skills/conversation-search/scripts/search_history.py --digest [DATE] [options]
```
...
python ~/.claude/skills/conversation-search/scripts/search_history.py "EMFILE error"

# Search for a feature implementation (last 3 days only)
python ~/.claude/skills/conversation-search/scripts/search_history.py --days 3 "spacedb reconstruction"

# Search within a specific project
python ~/.claude/skills/conversation-search/scripts/search_history.py "apriso recovery" --project "C:\Users\ee01287\Documents\Projects\Krok"

# Get JSON output for programmatic use
python ~/.claude/skills/conversation-search/scripts/search_history.py --digest today --format json
```
...
```
## March 05, 2026 - 4 sessions

### 1. VD154 Reconstruction Planning
   Session: `2da9ab0b`
   Branch: `development`
   Files: spacedb_explorer.rs, CLAUDE.md
   Commands: 6 executed

### 2. GAGEpack Import Wave 6
   Session: `96d4355d`
   Branch: `development`
   Files: _import_wave6_unified.py
   Commands: 2 executed
```
...
3. If more detail needed, read the full JSONL file:
   ```bash
   head -100 ~/.claude/projects/<encoded-path>/<session-id>.jsonl
   ```
